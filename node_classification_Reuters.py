...
my_subs = pd.Series(labels).rename('subject')

train_subjects = my_subs[:len(train_labels)]
test_subjects = my_subs[len(train_labels):]

val_subjects, test_subjects = model_selection.train_test_split(
    test_subjects, train_size=2180, test_size=None, stratify=test_subjects
)
...
ax.set(
    ...
    xlabel="$X_1$",
    ylabel="$X_2$",
    title=f"{transform.__name__} visualization of GCN embeddings for R8 dataset",
)